...
select s.SId, s.Sname, IFNULL(t.count_course, 0) as count_course, IFNULL(t.sum_score, 0) as sum_score
from Student as s
    left join (
        select SId, count(CId) as count_course, sum(score) as sum_score
        from SC
...
# 查有成绩的学生信息
# 仔细区分IN和EXISTS的应用场景
select * from Student as s where SId in ( select SId from SC );
# 或
select *
from Student as s
where
...
# 查询“李”姓老师的数量
# 聚合函数和正则表达式的简单使用
select count(*) from Teacher as t where t.Tname like '李%';
...
from SC as sc
    ...
    inner join Student as s on sc.CId = sc01.CId
    and s.SId = sc.SId;
    ...
    SId in (
        ...
        from ( # 获取全部成绩数据，course_count为和01同样的课程数目
            ...
            inner join (
                ...
                where
                    SId = '01'
            ) as t2 on t1.course_count = t2.cnt
    );
# n重子查询（不推荐）
SELECT *
FROM Student
...
where
    s.SId not in(
        # 查询上过张三老师（数学课）的学生名单
        select sc.SId
        from SC as sc, Course as c, Teacher as t
        where
            sc.CId = c.CId
            and c.TId = t.TId
            and t.Tname = '张三'
...
# 使用左联结，不然最高分会消失
select sc1.CId, sc1.SId, sc1.score, count(sc1.score) as score_grade
from SC as sc1
    left join SC as sc2 on sc1.CId = sc2.CId
    and sc1.score < sc2.score
group by
    sc1.CId,
    sc1.SId,
    sc1.score
order by sc1.CId, score_grade;
...
select sc1.CId, sc1.SId, sc1.score, count(sc2.score) + 1 as score_grade
from SC as sc1
    left join SC as sc2 on sc1.CId = sc2.CId # 同科
    and sc1.score < sc2.score
group by
    sc1.CId,
    sc1.SId,
...
select sc1.CId, sc1.SId, sc1.score, count(sc2.score) + 1 as 'c_score_rank'
from SC as sc1
    left join SC as sc2 on sc1.CId = sc2.CId # 这两行是关键，保留比当前成绩高的记录，这样就很容易算出当前成绩是多少名了
    and sc1.score < sc2.score
group by
    sc1.SId,
    sc1.CId,
...
# 查询男生、女生人数
# 方案1：分组count，产生纵向表
select s.Ssex, count(s.SId) from Student as s group by s.Ssex;
# 方案2：case，产生横向表
select sum(IF(s.Ssex = '男', 1, 0)) as 'male_stu', sum(IF(s.Ssex = '女', 1, 0)) as 'female_stu'
from Student as s;
...
# 查询1990年出生的学生名单
# 使用标准函数year()
select s.* from Student as s where year(s.Sage) = 1990;
...
from Course as c
    inner join SC as sc on c.CId = sc.CId
group by
    c.CId,
    c.Cname
order by avg_score desc, c.CId;
...
from
    Student as s
    inner join SC as sc
    inner join Course as c on s.SId = sc.SId
    and sc.CId = c.CId
    and c.Cname = '数学'
where
    sc.score < 60;
...
SELECT DISTINCT
    sc1.*
FROM SC AS sc1
    INNER JOIN SC AS sc2 ON sc1.score = sc2.score
WHERE
    sc1.cid <> sc2.cid;
...
# 聚合函数的基本考察
select CId, count(SId) as 'count_stu'
from SC
group by
    CId
having
